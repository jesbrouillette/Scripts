...
#		If the user is running Word or Excel the will be notified of the shutdown.  They are then allowed to continue to work, but will be reminded every 5 min.  If they choose "Ok" the will be logged out.  If not the script will continue to check every 5 minutes for the existence of the open Office applications.
...
param (
	...
	[ValidatePattern('.*\.exe$')]
	[string]$global:trigger = "calc.exe",
	...
	[array]$global:waitfor = @("winword","excel"),
...
#	$global:force = $force
	...
	#SourceIdentifier name.  This will allow multiple instances of the script to monitor for multiple applications.
